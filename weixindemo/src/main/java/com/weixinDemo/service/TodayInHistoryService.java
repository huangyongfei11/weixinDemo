...
public class TodayInHistoryService {
    ...
    private static String extract(String html) {
        ...
        Pattern p = Pattern.compile("(.*)(<div class=\"nls\">)(.*?)(</div>)(.*)");
        ...
        if (m.matches()) {
            ...
            // 抽取需要的数据
            for (String info : m.group(3).split("</li>")) {
                info = info.replace(dateTag, "").replace("<div>","").replace("<ul>","").replace("<li>"," ").replaceAll("</?+>", "").trim();
                info = info.replace(dateTag, "").replace(" \" "," \\").replace("[", "").replaceAll("]", "").trim();
                // 在每行末尾追加2个换行符
                if (!"c".equals(info)) {
    ...
    public static String getTodayInHistoryInfo() {
        // 获取网页源代码
        String html = httpRequest("http://jintian.cidianwang.com//");
        // 从网页中抽取信息
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        System.out.println(info.getBytes("utf-8").length);
